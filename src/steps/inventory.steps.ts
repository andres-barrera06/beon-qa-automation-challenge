...
export class InventorySteps {
  constructor(private inventoryPage: InventoryPage, private logger: Logger) {}

  async addItemsToCart(indexes: number[]) {
    await test.step(`Add ${indexes.length} items to cart`, async () => {
      this.logger.step(`Adding items at indexes [${indexes.join(', ')}] to cart`);
  ...
  async navigateToCart() {
    await test.step('Navigate to shopping cart', async () => {
      this.logger.step('Navigating to shopping cart');
